...
export default function TestResultsPage() {
  ...
  const fetchStudentsData = async () => {
    try {
      const response = await authApiClient.get(`${BASE_URL}/api/v1/admin/UserTestSessions`, {
        params: {
              ...
              {students.map((student, index) => (
                <tr key={student.userTestSessionId} className={index % 2 === 0 ? 'bg-white' : 'bg-[var(--Bg-main)]'}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 flex space-x-2">
                    <button
                      onClick={() => router.push(`/pages/admin/test-result-details?userTestSessionId=${student.userTestSessionId}`)}
                      title="مشاهده جزئیات"
